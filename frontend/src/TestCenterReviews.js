...
class TestCenterReviews extends React.Component {
	constructor(props){
		super(props);
		...
		this.test_center_id = this.props.match.params.id;
	...
	componentDidMount() {
		const url = process.env.REACT_APP_API_URL + "/test_center_reviews/" + this.test_center_id;
	...
	formSubmit(e) {
		...
		const data = {
			...
			"center_id": this.test_center_id
		};
		const url = process.env.REACT_APP_API_URL + "/test_center_reviews";
...
export default TestCenterReviews;